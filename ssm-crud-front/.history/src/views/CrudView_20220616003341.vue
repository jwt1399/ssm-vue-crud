<template>
    ...
    <!-- <Dialog ref="dialog"/> -->
        ...
        <el-form :model="EmpForm" :rules="rules" ref="formRef" label-position="right" label-width="80px" status-icon>
...
const handleDelete = (index, row) => {
    ...
    ElMessageBox.confirm('此操作将永久删除该信息, 是否继续?','提示',{
        ...
        console.log(deleteEmployee(row.empId))
    }).catch(() => {
...
function getEmployee(empId) {
    axios.get(`api/ssm_crud_back/emp/${empId}`)
    .then(response => {
...
const handleEdit = (index, row) => {
    ...
    console.log("测试"+getEmployee(row.empId))
